...
def create_eda_data_point(eda_len: int, true_timestamp: str,
                          ...
                          measurement_timestamp_utc: str) -> schemas.Eda:
...
class UnpackDataTest(unittest.TestCase):
    ...
    def test_estimated_sampling_frequency_missing_packet_drop_nan(self):
        data_points = [
            create_ppg_data_point(100, '2022-01-01 00:00:00', 10, 100, 1,
                                  '2022-01-01 00:00:00'),
            create_ppg_data_point(100, '2022-01-01 00:00:01', 10, 100, 1,
    ...
    def test_estimated_sampling_frequency_missing_samples_in_packet_drop_nan(
            self):
        data_points = [
            create_ppg_data_point(100, '2022-01-01 00:00:00', 10, 100, 1,
                                  '2022-01-01 00:00:00'),
    ...
    def test_extreme_difference_from_nominal_fs_error(self):
        ...
        with self.assertRaisesRegex(ValueError, error_msg_regex):
            ds_unpack.unpack_data_frame(test_df, ['green'], use_numba=True)
    ...
    def test_extreme_difference_from_nominal_fs_error_ignore(self):
        ...
        with self.assertWarns(UserWarning):
            unpacked_df = ds_unpack.unpack_data_frame(
                test_df, ['green'], ignore_median_fs_error=True, use_numba=True)
    ...
    def test_massive_first_packet(self):
        ...
        # Excess samples mess up the fs_est of the second packet, which replaced
        # with fs_med samples
        self.assertEqual(unpacked_df.shape[0], 400)

    def test_massive_first_packet_drop_nan(self):
        data_points = [
            create_ppg_data_point_dict(200, '2022-01-01 00:00:00', 10, 100, 1),
            create_ppg_data_point_dict(100, '2022-01-01 00:00:01', 10, 100, 1),
            create_ppg_data_point_dict(100, '2022-01-01 00:00:02', 10, 100, 1),
        ...
        # dropped
        self.assertEqual(unpacked_df.shape[0], 300)
    ...
    def test_massive_middle_packet(self):
        ...
        unpacked_df = ds_unpack.unpack_data_frame(test_df, ['green'],
                                                  use_numba=True)
        # excessive sample rate middle packets are replaced with fs_med samples
        self.assertEqual(unpacked_df.shape[0], 400)

    def test_massive_middle_packet_drop_nan(self):
        data_points = [
            create_ppg_data_point_dict(100, '2022-01-01 00:00:00', 10, 100, 1),
            create_ppg_data_point_dict(100, '2022-01-01 00:00:01', 10, 100, 1),
            create_ppg_data_point_dict(200, '2022-01-01 00:00:02', 10, 100, 1),
        ...
        unpacked_df = ds_unpack.unpack_data_frame(test_df, ['green'],
                                                  use_numba=True,
                                                  drop_nan=True)
        # excessive sample rate middle packets are dropped
        self.assertEqual(unpacked_df.shape[0], 300)
    ...
    def test_numba_vs_no_numba(self):
        data_points = [
            create_ppg_data_point(
                100,
                f'2022-01-01 00:{m:02d}:{s:02d}',
                10,
                100,
                1,
                measurement_timestamp_utc=f'2022-01-01 00:{m:02d}:{s:02d}',
            ) for m in range(60) for s in range(60)
        ]

        with TestPipeline() as p:
            input_pcol = (p | beam.Create(data_points) |
                          BuildDataPointDataFrames.PerParticipantDevice())
            _ = (input_pcol | 'unpack no_numba' >> beam.Map(
                ds_unpack.unpack_data_frame, ['green'], use_numba=False) |
                 'no_numba to str' >>
                 beam.Map(lambda df: df.to_csv('no_numba.csv')))
            _ = (input_pcol | 'unpack yes_numba' >> beam.Map(
                ds_unpack.unpack_data_frame, ['green'], use_numba=True) |
                 'yes_numba to str' >>
                 beam.Map(lambda df: df.to_csv('yes_numba.csv')))

        with open('no_numba.csv', 'r', encoding='UTF-8') as f:
            no_numba_csv = f.read()

        with open('yes_numba.csv', 'r', encoding='UTF-8') as f:
            yes_numba_csv = f.read()

        os.remove('no_numba.csv')
        os.remove('yes_numba.csv')

        self.assertEqual(no_numba_csv, yes_numba_csv)
