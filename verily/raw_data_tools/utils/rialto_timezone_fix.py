...
def _key_utc_offset_by_device(
        elem: Tuple[Timestamp, Any]) -> Tuple[str, float]:
    """Extract (device_serial, utc_offset_seconds) from (timestamp, data_source).

    data_source is expected to have .device.serial_number and
    .device.time_zone_name attributes (e.g. a types_pb2.DataSource).
    """
